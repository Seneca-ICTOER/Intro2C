...
flowchart
  subgraph title1[Human Language]
      subgraph title2[Programming Language]
        title3([Machine Language]):::title
      end
    end
    title1:::header1
    title2:::header2
    title3:::header3
    classDef header1 font-size: 12px, font-weight: 600;
    classDef header2 font-size: 10px, font-weight: 600, fill:lightblue;
    classDef header3 font-size: 9px, font-weight: 600, fill:orange;
```
...
```mermaid
%%{ init: { 'flowchart': { 'curve': 'stepBefore', 'nodeSpacing': 75 } } }%%
graph LR
    C ---> E
    subgraph <h2>Compiling</h2>
    A[Source Code]:::cProgram --> B[Compiler]:::compiler
    B --> C[Binary Code]:::machine
    end
    subgraph <h2>Executing</h2>
    D[Input]:::user ---> E[Binary Code]:::cProgram
    E ---> F[Output]:::user
    end

    classDef cProgram fill:#bef;
    classDef machine fill:#fd8;
    classDef compiler fill:#fe6, width:100px;
    classDef user fill:#4b7, width:80px;
```
...
<div style={{ display: 'flex', gap: '1em' }}>
  <TextBox color="#bef">C Program</TextBox>
  <TextBox color="#fe6">Compiler</TextBox>
  <TextBox color="#fd8">Machine Language Program</TextBox>
  <TextBox color="#4b7">User</TextBox>
</div>
...
```mermaid
%%{ init: { 'flowchart': { 'diagramPadding': 45 } } }%%
graph LR
  A[hello.c]:::cProgram ---> B[gcc]:::compiler ---> C[a.out]:::machine
  classDef cProgram width: 90px, fill:lightblue;
  classDef compiler width: 90px, fill:yellow;
  classDef machine width: 90px, fill:#ffd580;
```
...
```mermaid
%%{ init: { 'flowchart': { 'diagramPadding': 45, 'nodeSpacing': 100 } } }%%
flowchart LR
  A[hello.c]:::cProgram ---> B[cl]:::compiler ---> C[hello.exe]:::machine
  classDef cProgram width: 90px, fill:lightblue;
  classDef compiler width: 90px, fill:yellow;
  classDef machine width: 90px, fill:#ffd580;
```
